/**
 * School Portal Configuration — WORKER-SIDE MIRROR
 *
 * This is a server-side copy of client/src/config/schoolPortal.ts.
 * The Worker needs its own copy because it now validates access codes and
 * generates the redirect itself, instead of trusting the browser to do it.
 *
 * IMPORTANT: If you add, remove, or change a district's code or SharePoint
 * link, update BOTH this file and client/src/config/schoolPortal.ts, or the
 * portal and the tracking system will fall out of sync with each other.
 *
...
export const portalConfig: Record<string, string> = {
  // ─────────────────────────────────────────────────────────────────────────
